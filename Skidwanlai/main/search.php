...
<!-- == START OF LIST LAYOUT == -->
									...
									<div class="container">
										<div class="breadcrumb-inner">
											<ul class="list-inline list-unstyled">
												...
												<li class='active'><?php echo $_GET["search_input"];?></li>
											</ul>
									...
									if (!isset($_GET['sortby'])){
										$query_items = mysqli_query($con, "SELECT *,SUM(stock) as tStock, MIN(daterestock) AS mindaterestock FROM tbl_prod WHERE name LIKE '%$_GET[search_input]%' OR categoryname LIKE '%$_GET[search_input]%' OR brandname LIKE '%$_GET[search_input]%' GROUP BY code;");
									}
									else {
										if ($_GET['sortby']==1){
											$query_items = mysqli_query($con, "SELECT *,SUM(stock) as tStock, MIN(daterestock) AS mindaterestock FROM tbl_prod WHERE name LIKE '%$_GET[search_input]%' OR categoryname LIKE '%$_GET[search_input]%' OR brandname LIKE '%$_GET[search_input]%' GROUP BY code ORDER BY price ASC;");
										}
										else {
											$query_items = mysqli_query($con, "SELECT *,SUM(stock) as tStock, MIN(daterestock) AS mindaterestock FROM tbl_prod WHERE name LIKE '%$_GET[search_input]%' OR categoryname LIKE '%$_GET[search_input]%' OR brandname LIKE '%$_GET[search_input]%' GROUP BY code ORDER BY price DESC;");
										}
									}
									?>
											...
											<div class="dataTables_length" id="dataTables-customerOrder_length">
												...
												if (!isset($_GET['sortby'])){ ?>
													<select name="sortby" id="sortby" style="display: inline; width: auto;" class="form-control input-sm">
														...
														<option value="search.php?search_input=<?php echo $_GET['search_input'];?>&sortby=1">Price: Lowest - Highest</option>
														<option value="search.php?search_input=<?php echo $_GET['search_input'];?>&sortby=2">Price: Highest - Lowest</option>
													</select>
												<?php
												}
												else {
...
<tbody>
<?php
if (mysqli_num_rows($query_items)<1){
?>
<h3 class="name">No item matches <?php echo $_GET['search_input']; ?></h3>
<?php	
}
else {
...
<span style="font-size: 16px; font-family: 'FjallaOneRegular'; line-height: 18px; text-transform: uppercase; color: #666666; padding: 0px; font-weight: normal;">Availability :</span>&emsp;
<?php
...
?>
<span style="color: #ff6c6c; font-size: 20px;">Out of stock</span>
<?php
}
?>
</div>
