							...
							<div class="tab-pane active"  id="list-container">
								...
								<?php
									if (!isset($_GET['sortby'])){
										$query_items = mysqli_query($con, "SELECT *,SUM(stock) as tStock, MIN(daterestock) as mindaterestock FROM tbl_prod WHERE categoryname!='Package' GROUP BY code;"); }
									else {
										if($_GET['sortby']==1){
											$query_items = mysqli_query($con, "SELECT *,SUM(stock) as tStock, MIN(daterestock) as mindaterestock FROM tbl_prod WHERE categoryname!='Package' GROUP BY code ORDER BY price ASC;"); }
										else {
											$query_items = mysqli_query($con, "SELECT *,SUM(stock) as tStock, MIN(daterestock) as mindaterestock FROM tbl_prod WHERE categoryname!='Package' GROUP BY code ORDER BY price DESC;"); }
									} ?>
									<div class="dataTable_wrapper">
										<div class="dataTables_length" id="dataTables-customerOrder_length">
											...
											if (!isset($_GET['sortby'])){ ?>
												...
												<option value="allprod.php?sortby=1">Price: Lowest - Highest</option>
												<option value="allprod.php?sortby=2">Price: Highest - Lowest</option>
												</select><?php }
											else {
												if ($_GET['sortby']==1){ ?>
													<select name="sortby" id="sortby" style="display: inline; width: auto;" class="form-control input-sm">
										...
										<table class="table table-hover" id="dataTables-customerOrder">
						                    ...
						                    <tbody>
											<?php
							//$tbl_order = mysqli_query($con, "SELECT *, SUM(qty) AS sum_qty FROM tbl_order WHERE accid=$_SESSION[accid] GROUP BY invoice;");
											if ($query_items){
																...
																<div class="products">
																	<div class="product-list product">
																		<div class="row product-list-row">
																			<div class="product-info" style="padding: 0px 0px 20px 40px;">
																				<h3 class="name"><a href="allprod_detail.php?categoryname=<?php echo $row_items['categoryname'];?>&brandname=<?php echo $row_items['brandname'];?>&code=<?php echo $row_items['code'];?>"><?php echo $row_items['name'];?></a></h3>
																	...
																	<?php echo $row_items['des'];?></div>
																		<div class="cart clearfix animate-effect">
																			<div class="action">
																				<a href="allprod_detail.php?categoryname=<?php echo $row_items['categoryname'];?>&brandname=<?php echo $row_items['brandname'];?>&code=<?php echo $row_items['code'];?>" class="btn btn-primary">
